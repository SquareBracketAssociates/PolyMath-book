...
The class of the object implementing the bisection algorithm is a subclass of the abstract class \code{PMFunctionalIterator}.
The class \code{PMBisectionZeroFinder} needs the following additional instance variables:
\begin{itemize}
  \item \code{positiveX} $x_{pos}$ and
  \item \code{negativeX} $x_{neg}$
\end{itemize}
The bisection algorithm proper is implemented only within the method \code{evaluateIteration}.
...
suppressed.
Thus, the method \code{setFunction:} must also force a
...
returns:  \code{an OrderedCollection(-0.9999999999999998 -2.0 5.0)}\\

The methods needed to get the roots are shown in Listing \ref{lst:polroots}.

The deflation algorithm is implemented in the method \code{deflateAt:} using the iterator method \code{collect:} (\cf section \ref{sec:collect}).
