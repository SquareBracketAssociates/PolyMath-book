...
\ref{sec:siterrel}. The default creation class method \code{new}
...
class implementing trapeze integration. The method \code{evaluateIteration} is the only method needing change.
...
\end{itemize}
The method \code{evaluateIteration} (c.f. section
...
implementation of Romberg's algorithm. The class \code {PMRombergIntegrator} is a subclass of the class \code{PMTrapezeIntegrator} defined in section \ref{sec:strapeze}.
...
The sample points defining the interpolation are stored in an \code{OrderedCollection}. This collection is created in the method \code{computeInitialValues}. Since the number of points will never
exceed the order of the interpolation the maximum size is preset
when the collection is created. The method \code{computeInitialValues} also creates the object in charge of
